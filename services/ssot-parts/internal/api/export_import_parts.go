...
func exportAll(ctx context.Context, db *pgxpool.Pool, tenant string) (models.ExportPayload, error) {
	...
	for rows.Next() {
		var x models.Part
		if err := rows.Scan(&x.ID, &x.TenantID, &x.Name, &x.Category, &x.Puk, &x.SpecJSON, &x.CreatedAt, &x.UpdatedAt); err != nil {
			rows.Close()
			return p, err
		}
	...
	for rows.Next() {
		...
		if err := rows.Scan(&x.ID, &x.TenantID, &x.PartID, &x.DeviceModelID, &x.CreatedAt); err != nil {
			rows.Close()
			return p, err
		}
	...
	for rows.Next() {
		...
		if err := rows.Scan(&x.ID, &x.TenantID, &x.PartID, &x.VendorID, &x.SKU, &x.UnitPriceCents, &x.Currency, &x.LeadTimeDays, &x.CreatedAt, &x.UpdatedAt); err != nil {
			rows.Close()
			return p, err
		}
...
func importAll(ctx context.Context, db *pgxpool.Pool, tenant string, body map[string]any) (map[string]any, error) {
	...
	if len(parts) == 0 && len(compat) == 0 && len(vskus) == 0 {
	...
	err := withTx(ctx, db, func(tx pgx.Tx) error {
		...
		for _, it := range vskus {
			...
			vendor := trim(m["vendorId"])
			sku := trim(m["sku"])
			if vendor == "" || sku == "" {
				continue
			}
			unit := int64(0)
			if v, ok := m["unitPriceCents"].(float64); ok {
				unit = int64(v)
			}
			cur := trim(m["currency"])
			if cur == "" {
			...
			lt := 7
			if v, ok := m["leadTimeDays"].(float64); ok {
				lt = int(v)
			}
			_, err := tx.Exec(ctx, `
