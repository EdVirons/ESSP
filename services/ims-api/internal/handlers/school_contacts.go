...
func (h *SchoolContactsHandler) Create(w http.ResponseWriter, r *http.Request) {
	...
	if err := h.pg.SchoolContacts().Create(r.Context(), c); err != nil {
		http.Error(w, "failed to create contact", http.StatusInternalServerError); return
	}
	if c.IsPrimary {
...
func (h *SchoolContactsHandler) List(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil { http.Error(w, "failed to list contacts", http.StatusInternalServerError); return }
	writeJSON(w, http.StatusOK, map[string]any{"items": items})
}

type setPrimaryReq struct{ ContactID string `json:"contactId"` }

func (h *SchoolContactsHandler) SetPrimary(w http.ResponseWriter, r *http.Request) {
	schoolID := chi.URLParam(r, "schoolId")
	var req setPrimaryReq
	if err := json.NewDecoder(r.Body).Decode(&req); err != nil { http.Error(w, "invalid json", http.StatusBadRequest); return }
	if strings.TrimSpace(req.ContactID) == "" { http.Error(w, "contactId required", http.StatusBadRequest); return }
	tenant := middleware.TenantID(r.Context())
	if err := h.pg.SchoolContacts().SetPrimary(r.Context(), tenant, schoolID, strings.TrimSpace(req.ContactID)); err != nil {
		http.Error(w, "failed to set primary", http.StatusInternalServerError); return
	}
