...
func (r *BOQRepo) List(ctx context.Context, p BOQListParams) ([]models.BOQItem, string, error) {
	...
	argN := 3
	if p.Approved != nil {
		conds = append(conds, "approved=$"+itoa(argN))
		args = append(args, *p.Approved)
		argN++
	}
	if p.HasCursor {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&x.ID, &x.TenantID, &x.ProjectID, &x.Category, &x.Description, &x.PartID, &x.Qty, &x.Unit, &x.EstimatedCostCents, &x.Approved, &x.CreatedAt, &x.UpdatedAt); err != nil {
			return nil, "", err
		}
		out = append(out, x)
	}
