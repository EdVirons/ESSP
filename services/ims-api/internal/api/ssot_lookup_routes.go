...
func (s *Server) handleSchoolLookup(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil { httpx.Error(w, statusFromLookupErr(err), err.Error()); return }
...
func statusFromLookupErr(err error) int {
	if err == lookups.ErrNotFound { return 404 }
	if err == lookups.ErrSnapshotMissing { return 424 } // Failed Dependency
	return 500
}
