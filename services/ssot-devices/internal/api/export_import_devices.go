...
func exportAll(ctx context.Context, db *pgxpool.Pool, tenant string) (models.ExportPayload, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&x.ID,&x.TenantID,&x.Make,&x.Model,&x.Category,&x.SpecJSON,&x.CreatedAt,&x.UpdatedAt); err != nil { rows.Close(); return p, err }
	...
	for rows.Next() {
		var x models.Device
		if err := rows.Scan(&x.ID,&x.TenantID,&x.Serial,&x.AssetTag,&x.DeviceModelID,&x.SchoolID,&x.AssignedTo,&x.Lifecycle,&x.Enrolled,&x.CreatedAt,&x.UpdatedAt); err != nil { rows.Close(); return p, err }
...
func importAll(ctx context.Context, db *pgxpool.Pool, tenant string, body map[string]any) (map[string]any, error) {
	...
	for _, it := range modelsList {
		...
		id := trim(m["id"]); if id=="" { id=newID("dmodel") }
		make := trim(m["make"]); model := trim(m["model"])
		if make=="" || model=="" { continue }
		cat := trim(m["category"]); if cat=="" { cat="other" }
