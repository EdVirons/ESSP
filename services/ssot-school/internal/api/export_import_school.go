...
func exportAll(ctx context.Context, db *pgxpool.Pool, tenant string) (models.ExportPayload, error) {
	...
	for rows.Next() {
		var x models.County
		if err := rows.Scan(&x.ID,&x.TenantID,&x.Name,&x.Code,&x.CreatedAt,&x.UpdatedAt); err != nil { rows.Close(); return p, err }
	...
	for rows.Next() {
		...
		if err := rows.Scan(&x.ID,&x.TenantID,&x.CountyID,&x.Name,&x.Code,&x.CreatedAt,&x.UpdatedAt); err != nil { rows.Close(); return p, err }
	...
	for rows.Next() {
		...
		if err := rows.Scan(&x.ID, &x.TenantID, &x.Name, &x.Code, &x.CountyID, &x.SubCountyID, &x.Level, &x.Type, &x.Active,
			...
			&x.CreatedAt, &x.UpdatedAt); err != nil { rows.Close(); return p, err }
	...
	for rows.Next() {
		...
		if err := rows.Scan(&x.ID,&x.TenantID,&x.SchoolID,&x.Name,&x.Phone,&x.Email,&x.Role,&x.IsPrimary,&x.Active,&x.CreatedAt,&x.UpdatedAt); err != nil { rows.Close(); return p, err }
...
func importAll(ctx context.Context, db *pgxpool.Pool, tenant string, body map[string]any) (map[string]any, error) {
	...
	for _, it := range counties {
		...
		id := trim(m["id"]); if id=="" { id = newID("county") }
	...
	for _, it := range sub {
		m, ok := it.(map[string]any); if !ok { continue }
		id := trim(m["id"]); if id=="" { id = newID("subcounty") }
		name := trim(m["name"]); if name=="" { continue }
	...
	for _, it := range schools {
		m, ok := it.(map[string]any); if !ok { continue }
		id := trim(m["id"]); if id=="" { id = newID("school") }
		name := trim(m["name"]); if name=="" { continue }
		code := trim(m["code"])
		countyID := trim(m["countyId"])
		subID := trim(m["subCountyId"])
		level := trim(m["level"]); if level=="" { level="Other" }
		type_ := trim(m["type"]); if type_=="" { type_="public" }
		active := true
		if v, ok := m["active"].(bool); ok { active=v }
		...
		lat := 0.0
		if v, ok := m["latitude"].(float64); ok { lat=v }
		lng := 0.0
		if v, ok := m["longitude"].(float64); ok { lng=v }
		_, err := tx.Exec(ctx, `
	...
	for _, it := range contacts {
		m, ok := it.(map[string]any); if !ok { continue }
		id := trim(m["id"]); if id=="" { id = newID("contact") }
		schoolID := trim(m["schoolId"]); if schoolID=="" { continue }
		name := trim(m["name"]); if name=="" { continue }
		phone := trim(m["phone"])
		email := trim(m["email"])
		role := trim(m["role"]); if role=="" { role="point_of_contact" }
		isPrimary := false
